using System.Text; // Original script by Kneesnap, updated by Grossley
using System;
using System.IO;
...
HideProgressBar();
string isboth = "Sounds exported to " + winFolder + " in the 'Exported_Sounds' folder.";
if (Directory.Exists(winFolder + "External_Sounds\\")){
	isboth = "Sounds exported to " + winFolder + " in the 'Exported_Sounds' and 'External_Sounds' folders.";
}
ScriptMessage(isboth);
...
void DumpSound(UndertaleSound sound) {
	...
	if (!File.Exists(soundFilePath + audioExt) && !(skip == 1))
